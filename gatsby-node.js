...
exports.sourceNodes = async ({ actions, createNodeId, createContentDigest, store, cache }) => {
  ...
  await Promise.all(files.data.map(async (image) => {
    ...
    DirectusCMS_directus_files: {
      imageFile: {
        ...
        resolve: (source, args, context, info) => {
          const directusImageNode = context.nodeModel.getAllNodes({type: "DirectusImage"}).find(image => image.imageId == source.id);
          const fileNode = context.nodeModel.getNodeById({id: directusImageNode.localFile___NODE, type: "File"})
