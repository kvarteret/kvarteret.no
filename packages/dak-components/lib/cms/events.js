...
const getUpcomingEventsFromStudentBergen = async (date) => {
    ...
    const mapped = response.filter(x => new Date(x.event_end || x.start_date) >= date);
    return mapped;
  }
...
const getEventsAfter = async (lang, date) => {
  ...
  const upcomingEvents = await appendBase64Image(upcomingEventsCms.concat(upcomingEventsBergen).sort((a, b) => new Date(a.event_start) - new Date(b.event_start)));
